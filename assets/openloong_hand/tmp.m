clear variables;
close all;
robot=importrobot('AzureDragon_Simplified.urdf');
config=homeConfiguration(robot);
% robot2=importrobot('OGHR_wholeBody.urdf');
...
endTfL_fe=getTransform(robot,config,'Link_ankel_l_roll');
endTfR_fe=getTransform(robot,config,'Link_ankel_r_roll');
...
poseConst_fe_L = constraintPoseTarget('Link_ankel_l_roll');
...
poseConst_fe_R = constraintPoseTarget('Link_ankel_r_roll');
...
poseConst_hd_L.TargetTransform(1:3,4)=[-0.019, 0.34, -0.159];
...
poseConst_hd_R.TargetTransform(1:3,4)=[-0.019, -0.34, -0.159];
...
show(robot,configSol,'visual','on','collision','off');
