...
class OpenTeleVision:
    def __init__(self, img_shape, shm_name, queue, toggle_streaming, stream_mode="image", cert_file="./cert.pem", key_file="./key.pem", ngrok=False):
        ...
        if ngrok:
            ...
            self.app = Vuer(host='0.0.0.0', cert="teleop/cert.pem", key="teleop/key.pem", queries=dict(grid=False), queue_len=3)

        self.app.add_handler("HAND_MOVE")(self.on_hand_move)
        self.app.add_handler("CAMERA_MOVE")(self.on_cam_move)
        if stream_mode == "image":
            existing_shm = shared_memory.SharedMemory(name=shm_name)
            if existing_shm.size < (self.img_shape[0] * self.img_shape[1] * 3):
                raise ValueError("Shared memory buffer is too small for the requested array.")
    ...
    async def main_webrtc(self, session, fps=60):
        ...
        session.upsert @ WebRTCStereoVideoPlane(
                src="https://192.168.113.244:8080/offer",
                # iceServer={},
                key="zed",
                aspect=1.33334,
                height = 8,
...
#     shm = shared_memory.SharedMemory(create=True, size=np.prod(img_shape) * 3)
#
#     shm_name = shm.name
#     img_array = np.ndarray((img_shape[0], img_shape[1], 3), dtype=np.uint8, buffer=shm.buf)
#
#     # tv = OpenTeleVision(img_shape,shm_name,queue=Queue(),toggle_streaming=asyncio.Event(),stream_mode="image",cert_file="./cert.pem",key_file="./key.pem")
#     tv = OpenTeleVision(resolution_cropped, shm.name, queue=Queue(), toggle_streaming=asyncio.Event(), ngrok=True)
#     while True:
#         print("left landmarks: ",tv.left_landmarks)
#         print("left_hand" ,tv.left_hand)
