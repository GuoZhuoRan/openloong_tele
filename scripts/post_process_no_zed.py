...
def load_svo(path, crop_size_h=240, crop_size_w=320):
    ...
    nb_frames = zed.get_svo_number_of_frames()#episode-11:4543 frames
...
def find_all_episodes(path):
    episodes = [os.path.join(path, f) for f in os.listdir(path) if f.startswith('episode') ]
